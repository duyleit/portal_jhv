...
class BookmealController extends ControllerBase
{
    /**
     * Index action
     */
    ...
    public  function  initialize()
    {
     ...
     // parent::initialize();
        $this->auth();
        $this->view->mealdept = Bookdepartment::find   ([
            'columns' => 'bd_id,name',
            'order' => 'name '
        ]);
...
//
//        $bookmeal = Bookmeal::find($parameters);
//        if (count($bookmeal) == 0) {
//            $this->flash->notice("The search did not find any bookmeal");
//
...
//        $nguoidat = $this->session->get('user-fullname');
        ...
        $sql = "SELECT bookmeal.bm_id,date,user_code,dept_code,bookdepartment.name,lunch,lunch_add,lunch_veg,lunch_soup,lunch_veg_add,dinner,dinner_add,dinner_veg_add,supper,supper_veg,separate_table,buffet,note FROM Bookdepartment,Bookmeal where Bookmeal.dept_code= Bookdepartment.bd_id and status != -1";
         if ($date != '') {
...
//       var_dump($this->view->xtime);
        ...
        $db = $this->db;
        $sql = "SELECT bd_id,name FROM user,bookdepartment where bookdepartment.bd_code = user.dept_code and user = '". $this->session->get('user')."'";
 ...
 //      $this->tag->setDefault("mealorderdeptId",  $result[0]);
    ...
    public function editAction($bm_id)
    {
        if (!$this->request->isPost()) {
            ...
            if (!$bookmeal) {
                $this->flash->error("bookmeal was not found");
            ...
            $namedept=Bookdepartment::findFirst($bookmeal->dept_code);
            ...
            $this->tag->setDefault("dept_code",$namedept->bd_code);
...
//            $this->tag->setDefault("date_create", $bookmeal->date_create);
 //          $this->tag->setDefault("date_modify", $bookmeal->date_modify);
//            $this->tag->setDefault("user_code_modify", $bookmeal->user_code_modify);
            
        }
    }

    /**
     * Creates a new bookmeal
     */
    public function createAction()
    {
        ...
        date_default_timezone_set("Asia/bangkok");
        $bdid=Bookdepartment::findFirstByBd_code($this->request->getPost("dept_code"));
...
//        var_dump( $currentDateTime);
        ...
        if (!$bookmeal) {
            $this->flash->error("bookmeal does not exist " . $bm_id);
      ...
      //  $dayold= $bookmeal->date_create;
        $bd_id=Bookdepartment::findFirstByBd_code($this->request->getPost("dept_code"));
...
//       $bookmeal->dateCreate = $dayold;

       $bookmeal->dateModify = date('Y-m-d H:i:s');
//        $bookmeal->userCodeModify = $this->request->getPost("user_code_modify");
...
//                'size'  => 20,
        ...
        if($i<=$count) {
            ...
            $missdept = "select name from bookdepartment where bd_id not in(select distinct bookdepartment.bd_id  from bookdepartment,bookmeal where bookdepartment.bd_id = bookmeal.dept_code and date = '" . $date . "')";
