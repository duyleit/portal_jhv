...
<head>
    ...
    <link rel="stylesheet" href="<?= $this->url->get('vendor/Delete-Confirmation-Dialog/bootstrap-confirm-delete.css') ?>">

    
    <link rel="stylesheet" href="<?= $this->url->get('vendor/fullcalendar/packages/core/main.css') ?>">
    <link rel="stylesheet" href="<?= $this->url->get('vendor/fullcalendar/packages/daygrid/main.css') ?>">
        ...
        <div class="container">
            ...
            <div class="collapse navbar-collapse" id="navbarsExample07">
                <ul class="navbar-nav ml-auto small">
                    <li class="nav-item">
                        <a class="nav-link<?php if ($this->dispatcher->getControllerName() == 'index') { ?> active<?php } ?>" href="<?= $this->url->get('') ?>">Trang Chủ<br />首頁<span class="sr-only">(current)</span></a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link<?php if ($this->dispatcher->getControllerName() == 'gm') { ?> active<?php } ?>" href="<?= $this->url->get('gm') ?>">Ban Tổng Giám Đốc<br />總經理部</a>
                    </li>
                    <li class="nav-item dropdown">
                        <a class="nav-link<?php if ($this->dispatcher->getControllerName() == 'hr') { ?> active<?php } ?>" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">Bộ Phận Nhân Sự<br />人力資源</a>
                        ...
                        <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                            ...
                            <a class="dropdown-item" href="<?= $this->url->get('post/group/3') ?>">Lưu Trình, Quy Trình(cho người Việt) VN HR程序文件</a>
                            
                            
                            
                            <a class="dropdown-item" href="<?= $this->url->get('post/group/7') ?>">Tạp Chí Công Ty 公司月刊</a>
                            <div class="dropdown-divider"></div>
                            <a class="dropdown-item" href="<?= $this->url->get('post/group/9') ?>">Quyết định 決定書</a>
                            <a class="dropdown-item" href="<?= $this->url->get('post/group/10') ?>">Thông báo 通報</a>
                        </div>
                    </li>
                    ...
                    <li class="nav-item">
                        <a class="nav-link<?php if ($this->dispatcher->getControllerName() == 'sb') { ?> active<?php } ?>" href="<?= $this->url->get('sb') ?>">Hộp Thư Góp Ý<br />意見箱</a>
                    </li>
                    <li class="nav-item">
                        <a class="nav-link<?php if ($this->dispatcher->getControllerName() == 'search') { ?> active<?php } ?>" href="<?= $this->url->get('post/search/item') ?>">Tìm kiếm<br />搜索</a>
                    </li>
                    ...
                    <?php if ($this->session->get('user') != null) { ?>
                        <li class="nav-item dropdown">
                            ...
                            <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                                <a class="dropdown-item" href="<?= $this->url->get('post/detail/2') ?>">Hướng dẫn 用戶指南</a>
                                <div class="dropdown-divider"></div>
                    ...
                    <?php } else { ?>
                        <li class="nav-item">
                            <a class="nav-link" href="<?= $this->url->get('../user/auth/login') ?>">Đăng Nhập<br />登录</a>
                        </li>
                    <?php } ?>
                </ul>
            </div>
...
<script type="text/javascript" src="<?= $this->url->get('vendor/Delete-Confirmation-Dialog/bootstrap-confirm-delete.js') ?>"></script>
<script type="text/javascript" src="<?= $this->url->get('vendor/ckeditor/ckeditor.js') ?>"></script>


<script type="text/javascript" src="<?= $this->url->get('vendor/fullcalendar/packages/core/main.js') ?>"></script>

<script type="text/javascript" src="<?= $this->url->get('vendor/fullcalendar/packages/daygrid/main.js') ?>"></script>
...
<script type="text/javascript">
    $(document).ready(function(e) {
        ...
        CKEDITOR.replace( 'fieldContent', {
            filebrowserBrowseUrl: "<?= $this->url->get('vendor/kcfinder/browse.php?opener=ckeditor&type=files') ?>",
            filebrowserImageBrowseUrl: "<?= $this->url->get('vendor/kcfinder/browse.php?opener=ckeditor&type=images') ?>",
            filebrowserFlashBrowseUrl: "<?= $this->url->get('vendor/kcfinder/browse.php?opener=ckeditor&type=flash') ?>",
            filebrowserUploadUrl: "<?= $this->url->get('vendor/kcfinder/upload.php?opener=ckeditor&type=files') ?>",
            filebrowserImageUploadUrl: "<?= $this->url->get('vendor/kcfinder/upload.php?opener=ckeditor&type=images') ?>",
            filebrowserFlashUploadUrl: "<?= $this->url->get('vendor/kcfinder/upload.php?opener=ckeditor&type=flash') ?>"
        });
        <?php } ?>
    ...
    function openKCFinder(field) {
        ...
        window.open('<?= $this->url->get('vendor/kcfinder/browse.php?type=images&dir=files/public') ?>', 'kcfinder_textbox',
    ...
    <?php if ($this->router->getControllerName() == 'gm') { ?>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventLimit: true,
                events: {
                    url: '<?= $this->url->get('gm/calendarevent') ?>',
                    failure: function() {
